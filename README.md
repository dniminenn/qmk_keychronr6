...
QMK firmware for the Keychron R6 (ANSI, RGB). Everything stock does, wired,
Bluetooth, and 2.4GHz, and it works with **both VIA and the Keychron
Launcher**, including per-key RGB and lock indicator colors.

Reverse-engineered from a retail unit. Not affiliated with Keychron. This repo
is just the board overlay: drop it into a Keychron QMK fork and build.
...
Prebuilt firmware (and a stock restore image) is on the Releases page.
...
2.4GHz, and battery keycodes. Drafts don't survive a page reload, so re-load
the JSON after refreshing, and don't leave the Keychron Launcher open in
another tab while VIA is connected.
